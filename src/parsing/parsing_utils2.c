...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/09/11 20:48:31 by lpellegr          #+#    #+#             */
/*   Updated: 2024/09/11 20:48:32 by lpellegr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	apply_clean_on_args(t_data *node, t_all *all)
{
	...
	if (node->cmd)
	{
		node->cmd = replace_var_on_arg(node->cmd, all);
		node->cmd = clean_arg(node->cmd);
	}
	while (node->arg && node->arg[i])
	{
		node->arg[i] = replace_var_on_arg(node->arg[i], all);
